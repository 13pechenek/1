...
# vwall_shape(установка вертикальных стен)
# hwall_sеt(установка вертикальных стен)
...
while running:
    ...
    if hwall_set and pygame.mouse.get_pressed()[0] and not hwall_shape:
        ...
        n.append(m[1])
        n.append(100)
        hwall = Walls.HorizontalObstacles(n)
        walls.append(hwall)
        hwall_shape = True
        h += 1
    ...
    if vwall_set and pygame.mouse.get_pressed()[0] and not vwall_shape:
        ...
        n.append(m[1])
        n.append(100)
        vwall = Walls.VerticalObstacles(n)
        walls.append(vwall)
        vwall_shape = True
        v += 1
    ...
    for e in enemies:
        pygame.draw.rect(
            screen, (255, 0, 0),
            (e.x_enemy-5, e.y_enemy-5, 10, 10))
    for w in walls:
        pygame.draw.rect(
            screen, (255, 0, 0),
            (w.x_wall, w.y_wall, w.width, w.heith))
...
# название которому дается после нажатия esc(при выходе)
if not running:
    pygame.quit()
    st = input()+'.txt'
